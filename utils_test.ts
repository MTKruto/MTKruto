...
import { BetterDataView } from "./utils.ts";

Deno.test("BetterDataView", async (t) => {
  const dataView = new BetterDataView(
    // deno-fmt-ignore
  ...
  await t.step("setBigUint128", () => {
    const dataView = new BetterDataView(new Uint8Array(16).buffer);
