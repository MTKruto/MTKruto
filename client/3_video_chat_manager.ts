...
export class VideoChatManager implements UpdateProcessor<VideoChatManagerUpdate, true> {
  ...
  async joinVideoChat(id: string, params: string, params_?: JoinVideoChatParams): Promise<string> {
    ...
    const { updates } = await this.#c.invoke({
      ...
      call,
      join_as: params_?.joinAs ? await this.#c.getInputPeer(params_.joinAs) : { _: "inputPeerSelf" },
      params: { _: "dataJSON", data: params },
      invite_hash: params_?.inviteHash,
      muted: params_?.isAudioEnabled === false || undefined,
      video_stopped: params_?.isVideoEnabled === false || undefined,
