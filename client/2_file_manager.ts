...
export class FileManager {
  ...
  async #uploadStream(stream: ReadableStream<Uint8Array>, fileId: bigint, chunkSize: number, signal: AbortSignal | undefined) {
    ...
    for await (part of iterateReadableStream(stream.pipeThrough(new PartStream(chunkSize)))) {
      promises.push(this.#uploadPart(fileId, part.totalParts, !part.small, part.part, part.bytes, ms, signal));
    ...
    main: for (let part = 0; part < partCount;) {
      for (let i = 0; i < UPLOAD_POOL_SIZE; ++i) {
        for (let i = 0; i < UPLOAD_REQUEST_PER_CONNECTION; ++i) {
          ...
          promises.push(this.#uploadPart(fileId, partCount, isBig, part++, bytes, ms, signal));
          ms = Math.max(ms * .8, 0.003);
          if (promises.length == UPLOAD_POOL_SIZE * UPLOAD_REQUEST_PER_CONNECTION) {
            await Promise.all(promises);
            promises = [];
  ...
  async #uploadPart(fileId: bigint, partCount: number, isBig: boolean, index: number, bytes: Uint8Array, ms: number, signal: AbortSignal | undefined) {
    if (index > 0) {
      await delay(ms);
    }
    let retryIn = 1;
  ...
  async *downloadInner(location: Api.InputFileLocation, dcId: number, params: DownloadParams | undefined) {
    ...
    while (true) {
      promises.push(this.#downloadPart(dc, location, part++, offset, limit, id, ms, signal));
      ms = Math.max(ms * .8, 0.003);
      offset += BigInt(limit);
  ...
  async #downloadPart(dc: DC, location: Api.InputFileLocation, index: number, offset: bigint, limit: number, id: bigint | null, ms: number, signal: AbortSignal | undefined) {
    if (index > 0) {
      await delay(ms);
    }
    while (true) {
