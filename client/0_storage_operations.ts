...
export class StorageOperations {
  ...
  async getTlObject(keyOrBuffer: Api.AnyType | Uint8Array | readonly StorageKeyPart[]): Promise<Api.DeserializedType | null> {
    ...
    if (buffer !== null) {
      if (buffer instanceof Uint8Array) {
        return Api.deserializeType(X, rleDecode(buffer));
      } else if (Array.isArray(buffer)) {
        return Api.deserializeType(buffer[0], rleDecode(buffer[1]));
      } else {
        return buffer;
      }
    } else {
  ...
  async *iterFileParts(id: bigint, partCount: number, offset: number, signal: AbortSignal | undefined): AsyncGenerator<Uint8Array<ArrayBuffer>> {
    if (!this.#supportsFiles) {
      return;
    }
    for (let i = offset; i < partCount; i++) {
      signal?.throwIfAborted();
      const part = await this.#storage.get<Uint8Array<ArrayBuffer>>(K.cache.filePart(id, i));
      if (part === null) {
        continue;
      }
      yield part;
