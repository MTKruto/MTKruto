...
export class CallbackQueryManager implements UpdateProcessor<CallbackQueryManagerUpdate, true> {
  ...
  async answerCallbackQuery(id: string, params?: AnswerCallbackQueryParams) {
    ...
    await this.#c.invoke({ _: "messages.setBotCallbackAnswer", url: params?.url, query_id: BigInt(id), cache_time: params?.cacheTime ?? 0, message: params?.text, alert: params?.isAlert || undefined });
  }
  ...
  async handleUpdate(update: CallbackQueryManagerUpdate): Promise<Update> {
    return { type: "callbackQuery", callbackQuery: await constructCallbackQuery(update, this.#c.getPeer, this.#c.messageManager.getMessageWithReply.bind(this.#c.messageManager), this.#c.fileManager.getStickerSetName.bind(this.#c.fileManager), this.#c.getCommunity) };
  }
}
