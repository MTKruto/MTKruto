...
export class InlineQueryManager implements UpdateProcessor<InlineQueryManagerUpdate> {
  ...
  async answerInlineQuery(id: string, results: InlineQueryResult[], params?: AnswerInlineQueryParams) {
    ...
    await this.#c.invoke({ _: "messages.setInlineBotResults", query_id: BigInt(id), results: await Promise.all(results.map((v) => inlineQueryResultToTlObject(v, this.#c.messageManager.parseText.bind(this.#c.messageManager), this.#c.messageManager.usernameResolver.bind(this.#c.messageManager), this.#c.messageManager.inputRichTextToInputRichMessage.bind(this.#c.messageManager)))), cache_time: params?.cacheTime ?? 300, private: params?.isPersonal || undefined, switch_webview: params?.button?.url ? ({ _: "inlineBotWebView", text: params.button.text, url: params.button.url }) : undefined, switch_pm: params?.button && params.button.startParameter ? ({ _: "inlineBotSwitchPM", text: params.button.text, start_param: params.button.startParameter }) : undefined, gallery: params?.isGallery || undefined, next_offset: params?.nextOffset });
  }
