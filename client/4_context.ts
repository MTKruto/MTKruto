...
export class Context {
  ...
  #getReplyTo = (isQuoted: boolean | undefined, chatId: number, messageId: number, isEphemeral: boolean | undefined): { receiverId?: number; messageThreadId?: number; directMessagesTopicId?: number; replyTo?: ReplyTo; callbackQueryId?: string } => {
    if (isEphemeral) {
  ...
  async reply(text: string, params?: Omit<SendMessageParams, "replyTo" | "messageThreadId" | "businessConnectionId"> & ReplyParams): Promise<MessageText> {
    const { chatId, messageId, businessConnectionId, isEphemeral } = this.#mustGetMsg();
    const replyTo = this.#getReplyTo(params?.isQuoted, chatId, messageId, isEphemeral);
