...
export class Context {
  ...
  #mustGetMsg() {
    if (this.msg !== undefined) {
      return { chatId: this.msg.chat.id, messageId: this.msg.id, businessConnectionId: this.msg.businessConnectionId, userId: this.msg.from.id, senderId: this.msg.from?.id, isEphemeral: !!this.msg.receiver };
    }
  ...
  #mustGetChatId() {
    if (this.chat) {
      return this.chat.id;
    } else if (this.update.type === "messageInteractions") {
  ...
  #getReplyTo = (isQuoted: boolean | undefined): { receiverId?: number; messageThreadId?: number; directMessagesTopicId?: number; replyTo?: ReplyTo; callbackQueryId?: string } => {
    if (this.update.type === "story") {
      return { replyTo: { type: "story", chatId: this.update.story.chat.id, storyId: this.update.story.id } };
    }

    const { chatId, messageId, isEphemeral } = this.#mustGetMsg();
    if (isEphemeral) {
      const receiverId = this.msg?.isOutgoing ? this.msg.receiver?.id : this.msg?.from.id;
      if (!receiverId) {
        unreachable();
      }
      return { receiverId, replyTo: this.msg?.isOutgoing ? undefined : { type: "ephemeralMessage", messageId }, callbackQueryId: this.callbackQuery?.id };
    }
  ...
  async addPollOption(messageId: number, option: InputPollOption): Promise<void> {
    const chatId = this.#mustGetChatId();
  ...
  async addStoriesToAlbum(albumId: number, storyIds: number[]): Promise<StoryAlbum> {
    const chatId = this.#mustGetChatId();
  ...
  async addStoryToAlbum(albumId: number, storyId: number): Promise<StoryAlbum> {
    const chatId = this.#mustGetChatId();
  ...
  async createStoryAlbum(name: string, storyIds: number[]): Promise<StoryAlbum> {
    const chatId = this.#mustGetChatId();
  ...
  async delete(): Promise<void> {
    let { chatId, messageId, userId, isEphemeral } = this.#mustGetMsg();
    if (isEphemeral) {
      userId = this.msg?.isOutgoing ? this.msg.receiver?.id : userId;
      if (!userId) {
        unreachable();
      }
  ...
  async disableChatHistoryForNewMembers(): Promise<void> {
    const chatId = this.#mustGetChatId();
  ...
  async editStory(storyId: number, params?: EditStoryParams): Promise<Story> {
    const chatId = this.#mustGetChatId();
    return await this.client.editStory(chatId, storyId, params);
  }
  ...
  async enableChatHistoryForNewMembers(): Promise<void> {
    const chatId = this.#mustGetChatId();
  ...
  async getMessageCounters(messageId: number): Promise<MessageCounters> {
    const chatId = this.#mustGetChatId();
  ...
  async getMessageReadDate(messageId: number): Promise<number> {
    const chatId = this.#mustGetChatId();
  ...
  async getMessagesCounters(messageIds: number[]): Promise<MessageCounters[]> {
    const chatId = this.#mustGetChatId();
    return await this.client.getMessagesCounters(chatId, messageIds);
  }

  /** Context-aware alias for {@link Client.getNotificationSettings}. */
  async getNotificationSettings(): Promise<NotificationSettings> {
    const chatId = this.#mustGetChatId();
    return await this.client.getNotificationSettings(chatId);
  }
  ...
  async getPollVoters(messageId: number, params?: GetPollVotersParams): Promise<PollVoterList> {
    const chatId = this.#mustGetChatId();
    return await this.client.getPollVoters(chatId, messageId, params);
  }

  /** Context-aware alias for {@link Client.getRecentActions}. */
  async getRecentActions(params?: GetRecentActionsParams): Promise<RecentActionsEntry[]> {
    const chatId = this.#mustGetChatId();
    return await this.client.getRecentActions(chatId, params);
  }
  ...
  async getStarTransactions(params?: GetStarTransactionsParams): Promise<StarTransactionList> {
    const chatId = this.#mustGetChatId();
  ...
  async getStoriesInAlbum(albumId: number, params?: GetStoriesInAlbumParams): Promise<AlbumStoryList> {
    const chatId = this.#mustGetChatId();
    return await this.client.getStoriesInAlbum(chatId, albumId, params);
  }
  ...
  async getStoryAlbums(): Promise<StoryAlbum[]> {
    const chatId = this.#mustGetChatId();
  ...
  /** Context-aware alias for {@link Client.getTonTransactions}. */
  async getTonTransactions(params?: GetTonTransactionsParams): Promise<TonTransactionList> {
    const chatId = this.#mustGetChatId();
    return await this.client.getTonTransactions(chatId, params);
  }
  ...
  async getTopics(params?: GetTopicsParams): Promise<TopicList> {
    const chatId = this.#mustGetChatId();
    return await this.client.getTopics(chatId, params);
  }

  /** Context-aware alias for {@link Client.getTopicsById}. */
  async getTopicsById(topicIds: number[]): Promise<TopicList> {
    const chatId = this.#mustGetChatId();
    return await this.client.getTopicsById(chatId, topicIds);
  }
  ...
  async joinChat(): Promise<ChatJoinResult> {
    const chatId = this.#mustGetChatId();
  ...
  async removeStoriesFromAlbum(albumId: number, storyIds: number[]): Promise<StoryAlbum> {
    const chatId = this.#mustGetChatId();
  ...
  async removeStoryFromAlbum(albumId: number, storyId: number): Promise<StoryAlbum> {
    const chatId = this.#mustGetChatId();
  ...
  async reorderStoriesInAlbum(albumId: number, storyIds: number[]): Promise<StoryAlbum> {
    const chatId = this.#mustGetChatId();
  ...
  async reply(text: string, params?: Omit<SendMessageParams, "replyTo" | "messageThreadId" | "businessConnectionId"> & ReplyParams): Promise<MessageText> {
    const chatId = this.#mustGetChatId();
    const businessConnectionId = this.msg?.businessConnectionId;
    const replyTo = this.#getReplyTo(params?.isQuoted);
  ...
  async replySecretAudio(audio: FileSource, params?: Omit<SendSecretAudioParams, "replyToMessageId"> & ReplyParams): Promise<void> {
    const { chatId, messageId } = this.#mustGetSecretMsg();
    const replyToMessageId = params?.isQuoted ? messageId : undefined;
  ...
  async replySecretSticker(sticker: Sticker, params?: Omit<SendSecretStickerParams, "replyToMessageId"> & ReplyParams): Promise<void> {
    const { chatId, messageId } = this.#mustGetSecretMsg();
    const replyToMessageId = params?.isQuoted ? messageId : undefined;
  ...
  async replySecretVideoNote(videoNote: FileSource, params?: Omit<SendSecretVideoNoteParams, "replyToMessageId"> & ReplyParams): Promise<void> {
    const { chatId, messageId } = this.#mustGetSecretMsg();
    const replyToMessageId = params?.isQuoted ? messageId : undefined;
  ...
  async replySecretVoice(voice: FileSource, params?: Omit<SendSecretVoiceParams, "replyToMessageId"> & ReplyParams): Promise<void> {
    const { chatId, messageId } = this.#mustGetSecretMsg();
    const replyToMessageId = params?.isQuoted ? messageId : undefined;
  ...
  async reportChat(reason: ReportReason, params?: ReportChatParams): Promise<void> {
    const chatId = this.#mustGetChatId();
    return await this.client.reportChat(chatId, reason, params);
  }

  /** Context-aware alias for {@link Client.reportEphemeralMessage}. */
  async reportEphemeralMessage(messageId: number, params?: ReportEphemeralMessageParams): Promise<ReportResult> {
    const chatId = this.#mustGetChatId();
    return await this.client.reportEphemeralMessage(chatId, messageId, params);
  }
  ...
  async reportStories(storyIds: number[], params?: ReportStoryParams): Promise<ReportResult> {
    const chatId = this.#mustGetChatId();
    return await this.client.reportStories(chatId, storyIds, params);
  }

  /** Context-aware alias for {@link Client.reportStory}. */
  async reportStory(storyId: number, params?: ReportStoryParams): Promise<ReportResult> {
    const chatId = this.#mustGetChatId();
  ...
  async retractVote(messageId: number): Promise<void> {
    const chatId = this.#mustGetChatId();
  ...
  /** Context-aware alias for {@link Client.saveRichTextDraft}. */
  async saveRichTextDraft(richText: InputRichText, params?: SaveRichTextDraftParams): Promise<void> {
    const chatId = this.#mustGetChatId();
    return await this.client.saveRichTextDraft(chatId, richText, params);
  }
  ...
  async sendScheduledMessages(messageIds: number[]): Promise<Message[]> {
    const chatId = this.#mustGetChatId();
  ...
  async sendSecretScreenshotNotification(messageIds: string[]): Promise<void> {
    const id = this.#mustGetSecretChatId();
  ...
  async setDefaultSendAs(sendAs: ID): Promise<void> {
    const chatId = this.#mustGetChatId();
  ...
  /** Context-aware alias for {@link Client.setNotificationSettings}. */
  async setNotificationSettings(params?: SetNotificationSettingsParams): Promise<void> {
    const chatId = this.#mustGetChatId();
    return await this.client.setNotificationSettings(chatId, params);
  }
  ...
  async setStoryAlbumName(albumId: number, name: string): Promise<StoryAlbum> {
    const chatId = this.#mustGetChatId();
  ...
  async summarizeText(messageId: number, params?: SummarizeTextParams): Promise<SummarizedText> {
    const chatId = this.#mustGetChatId();
  ...
  async translateMessage(toLanguage: string, messageId: number, params?: TranslateTextParams): Promise<TranslatedText> {
    const chatId = this.#mustGetChatId();
  ...
  async translateMessages(toLanguage: string, messageIds: number[], params?: TranslateTextParams): Promise<TranslatedText[]> {
    const chatId = this.#mustGetChatId();
