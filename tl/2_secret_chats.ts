...
import { type AnyObject, type AnyType, type Enums, schema, type Types } from "./1_secret_chats_api.ts";
...
export * from "./1_secret_chats_api.ts";
...
export function is<S extends keyof Types>(name: S, value: unknown): value is S extends keyof Types ? Types[S] : never {
  return is_(name, value, schema);
}
export function isOneOf<S extends keyof Types>(names: S[] | readonly S[], value: unknown): value is S extends keyof Types ? Types[S] : never {
