...
export function deserializeType<T extends (keyof Types) | "X" | string>(name: T, bufferOrReader: TLReader | Uint8Array, isFunctionResult = false): T extends keyof Types ? Types[T] : DeserializedType {
  const reader = bufferOrReader instanceof Uint8Array ? new TLReader(bufferOrReader) : bufferOrReader;
  return isFunctionResult ? reader.readResult(name, schema) : reader.readType(name, schema);
}
