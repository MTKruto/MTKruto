...
export function deserializeType<T extends (keyof Api.Types) | "X" | string>(name: T, bufferOrReader: TLReader | Uint8Array, isFunctionResult = false): T extends keyof Api.Types ? Api.Types[T] : DeserializedType {
  const reader = bufferOrReader instanceof Uint8Array ? new TLReader(bufferOrReader) : bufferOrReader;
  return isFunctionResult ? reader.readResult(name, Api.schema) : reader.readType(name, Api.schema);
}
