  ...
  async open() {
    ...
    try {
      ...
      if (negotiation[1] === AUTH_METHOD_USERNAME_PASSWORD && !this.#credentials) {
        ...
        await this.#writeToSocket(concat([new Uint8Array([VERSION_USERNAME_PASSWORD_AUTH, this.#credentials.username.length]), encodeText(this.#credentials.username), new Uint8Array([this.#credentials.password.length]), encodeText(this.#credentials.password)]));
